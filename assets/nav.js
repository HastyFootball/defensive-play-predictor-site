(function(){
  ...
  const authed = ['dashboard.html','prep-center.html','game-mode.html','staff.html','app.html','live-mode.html','import-center.html','analytics.html','reports.html','opponents.html','playbook.html'].includes(path);
  const publicLinks = `
  ...
  const coachLinks = `
    ...
    <a class="${path==='playbook.html'?'active':''}" href="playbook.html">Playbook</a>
    <a class="${path==='staff.html'?'active':''}" href="staff.html">Staff</a>
    <button onclick="signOut()">Sign out</button>`;
  document.write(`
  <nav><div class="nav-inner"><a class="brand" href="${authed?'dashboard.html':'index.html'}"><div class="mark">AA</div><div class="brand-name"><span>Analyst</span> Assist</div></a><div class="nav-links coach-nav">${authed ? coachLinks : publicLinks}</div></div></nav>
  `);
})();
