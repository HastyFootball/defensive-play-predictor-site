...
const video=$('filmVideo'),livePreview=$('livePreviewVideo'),frameCanvas=$('frameCanvas'),overlay=$('overlayCanvas'),stage=$('stage'),viewerCard=document.querySelector('.fil-viewer-card');
...
let clipRecording=false,clipTimer=null,clipStartedAt=0,clipFrames=[],captureTimes={pre:null,snap:null,end:null};
let dbPromise=null;
...
async function share(){
 try{
  ...
  video.srcObject=stream;livePreview.srcObject=stream;
  await Promise.all([video.play(),livePreview.play()]);
  $('emptyState').style.display='none';$('liveDock').hidden=false;$('sourceDot').classList.add('live');
  const track=stream.getVideoTracks()[0],settings=track.getSettings?.()||{};
  setText('sourceStatus','Hudl source connected');setText('sourceDetail',track?.label||'Shared film');setText('modePill','LIVE');
  const w=settings.width||video.videoWidth||0,h=settings.height||video.videoHeight||0,fps=settings.frameRate||0;
  const quality=w>=1600?'Excellent':w>=1200?'Good':w>=900?'Usable':'Low';
  $('sourceQuality').innerHTML=`<strong>${quality} source</strong><span>${w||'?'} × ${h||'?'}${fps?` at ${Math.round(fps)} fps`:''}. ${w<1200?'Share the Hudl tab/window and enlarge its video for better player detection.':'Good for analysis.'}</span>`;
  setText('liveFeedStatus',`${w||'?'}×${h||'?'} live`);
  setEnabled(true);$('recordClipBtn').disabled=false;setTimeout(resize,180);track.addEventListener('ended',stop);applyProfile();
 }catch(e){if(e.name!=='NotAllowedError')alert(`Unable to share film: ${e.message}`)}
}
function stop(){
 if(clipRecording)stopClipCapture(false);
 if(stream)stream.getTracks().forEach(t=>t.stop());stream=null;video.srcObject=null;livePreview.srcObject=null;
 setEnabled(false);$('recordClipBtn').disabled=true;$('liveDock').hidden=true;$('emptyState').style.display='flex';$('sourceDot').classList.remove('live');setText('sourceStatus','No film source');setText('sourceDetail','Choose a Hudl tab or separate Hudl window.');setText('modePill','IDLE')
}
...
function liveSnapshotCanvas(maxWidth=1280){
 const c=document.createElement('canvas'),vw=video.videoWidth||1920,vh=video.videoHeight||1080,scale=Math.min(1,maxWidth/vw);c.width=Math.max(1,Math.round(vw*scale));c.height=Math.max(1,Math.round(vh*scale));
 const ctx=c.getContext('2d',{willReadFrequently:true});ctx.drawImage(video,0,0,c.width,c.height);return c
}
function liveSnapshot(){return liveSnapshotCanvas().toDataURL('image/jpeg',.82)}
function updateThumb(id,source,timeId,when){const c=$(id),ctx=c.getContext('2d');c.width=220;c.height=124;ctx.clearRect(0,0,c.width,c.height);if(source){const sw=source.width||source.videoWidth,sh=source.height||source.videoHeight;if(sw&&sh){const sc=Math.min(c.width/sw,c.height/sh),w=sw*sc,h=sh*sc;ctx.drawImage(source,(c.width-w)/2,(c.height-h)/2,w,h)}}setText(timeId,when?when.toLocaleTimeString([], {hour:'numeric',minute:'2-digit',second:'2-digit'}):'Not captured')}
function captureSnap(){if(!stream)return alert('Share Hudl first.');const c=liveSnapshotCanvas();snapFrame=c.toDataURL('image/jpeg',.82);captureTimes.snap=new Date();updateThumb('snapThumb',c,'snapThumbTime',captureTimes.snap);setText('modePill','SNAP CAPTURED');setText('stageHint','Snap was captured from the moving Live Hudl Feed. Advance Hudl to the end and capture End—or record the whole play clip.');$('analyzePlayBtn').disabled=!(snapFrame&&endFrame)&&clipFrames.length<3}
function captureEnd(){if(!stream)return alert('Share Hudl first.');const c=liveSnapshotCanvas();endFrame=c.toDataURL('image/jpeg',.82);captureTimes.end=new Date();updateThumb('endThumb',c,'endThumbTime',captureTimes.end);setText('modePill','END CAPTURED');setText('stageHint','End was captured from the moving Live Hudl Feed. Press Analyze Captures.');$('analyzePlayBtn').disabled=!(snapFrame&&endFrame)&&clipFrames.length<3}
function captureClipFrame(){if(!clipRecording||!stream)return;const c=liveSnapshotCanvas(960);clipFrames.push({t:performance.now()-clipStartedAt,data:c.toDataURL('image/jpeg',.68)});if(clipFrames.length>=72)stopClipCapture(true)}
function startClipCapture(){if(!stream)return alert('Share Hudl first.');clipFrames=[];clipRecording=true;clipStartedAt=performance.now();$('recordClipBtn').textContent='Stop & Analyze Clip';$('recordClipBtn').classList.add('fil-danger');setText('modePill','RECORDING PLAY');setText('stageHint','Play the snap in Hudl now. Stop after the play. The frozen editing frame will stay still while the Live Hudl Feed keeps moving.');$('clipSummary').textContent='Recording…';$('clipDetail').textContent='Capturing frames from the live source.';captureClipFrame();clipTimer=setInterval(captureClipFrame,160)}
function stopClipCapture(autoAnalyze=true){if(!clipRecording)return;clipRecording=false;clearInterval(clipTimer);clipTimer=null;$('recordClipBtn').textContent='Record Play Clip';$('recordClipBtn').classList.remove('fil-danger');const seconds=clipFrames.length?clipFrames[clipFrames.length-1].t/1000:0;$('clipSummary').textContent=`${clipFrames.length} frames · ${seconds.toFixed(1)} sec`;$('clipDetail').textContent=clipFrames.length>=3?'Ready for multi-frame tracking.':'Clip too short—record at least 1 second.';$('analyzePlayBtn').disabled=clipFrames.length<3&&!(snapFrame&&endFrame);setText('modePill','CLIP READY');setText('stageHint','Clip captured from the moving live feed. Analyze now or review your pre-snap labels first.');if(autoAnalyze&&clipFrames.length>=3)analyzePlay()}
function toggleClipCapture(){clipRecording?stopClipCapture(true):startClipCapture()}
async function detectClipTracks(){
 const sample=clipFrames.length<=10?clipFrames:clipFrames.filter((_,i)=>i%Math.ceil(clipFrames.length/10)===0).slice(0,10),frames=[];
 for(let i=0;i<sample.length;i++){setProgress(82+Math.round(i/sample.length*12),`Tracking frame ${i+1}/${sample.length}`);frames.push(await detectOnImageData(sample[i].data))}
 if(frames.length<2)return{tracks:[],moves:[]};
 let tracks=frames[0].map((b,i)=>({id:i,last:b,obs:[b]})),nextId=tracks.length;
 for(let fi=1;fi<frames.length;fi++){
  const boxes=frames[fi],used=new Set();
  for(const tr of tracks){let best=null;for(let i=0;i<boxes.length;i++){if(used.has(i))continue;const b=boxes[i],d=dist({x:tr.last.x+tr.last.w/2,y:tr.last.y+tr.last.h*.9},{x:b.x+b.w/2,y:b.y+b.h*.9});if((!best||d<best.d)&&d<.10)best={i,b,d}}if(best){used.add(best.i);tr.last=best.b;tr.obs.push(best.b)}}
  boxes.forEach((b,i)=>{if(!used.has(i))tracks.push({id:nextId++,last:b,obs:[b]})})
 }
 const stable=tracks.filter(t=>t.obs.length>=Math.max(3,Math.floor(frames.length*.45))),moves=stable.map(t=>{const a=t.obs[0],b=t.obs[t.obs.length-1];return{from:a,to:b,dx:(b.x+b.w/2)-(a.x+a.w/2),dy:(b.y+b.h*.9)-(a.y+a.h*.9),d:dist({x:a.x+a.w/2,y:a.y+a.h*.9},{x:b.x+b.w/2,y:b.y+b.h*.9}),observations:t.obs.length}});
 return{tracks:stable,moves}
}
...
async function analyzePlay(){
 if(clipFrames.length<3&&(!snapFrame||!endFrame))return alert('Record a play clip, or capture both Snap and End from the live feed first.');
 try{
  setProgress(82,'Preparing post-snap tracking');let moves=[],tracked=0,method='two-frame';
  if(clipFrames.length>=3){const result=await detectClipTracks();moves=result.moves;tracked=result.tracks.length;method='multi-frame'}
  else{const a=await detectOnImageData(snapFrame),b=await detectOnImageData(endFrame);moves=nearestMatches(a,b);tracked=moves.length}
  const g=geometry();let along=0,lateral=0;if(moves.length){along=mean(moves.map(m=>m.dx*g.u.x+m.dy*g.u.y));lateral=mean(moves.map(m=>m.dx*g.v.x+m.dy*g.v.y))}
  const direction=Math.abs(lateral)<.012?'Middle':lateral<0?'Left':'Right',spread=median(moves.map(m=>m.d));let playType='Run';
  if(spread>.065&&Math.abs(along)>.035)playType='Pass';if((analysis?.counts?.receivers||0)>=4&&spread>.045)playType='Pass';
...
async function toggleFullscreenStage(){try{if(document.fullscreenElement)await document.exitFullscreen();else await viewerCard.requestFullscreen()}catch(e){alert(`Fullscreen could not open: ${e.message}`)}setTimeout(resize,80)}
...
$('floatingUndoBtn').onclick=undoLastEdit;
$('floatingSaveBtn').onclick=()=>savePlay(false);
$('floatingRoleSelect').onchange=()=>{if(!selectedPerson)return;pushEditHistory();setLockedRole(selectedPerson,$('floatingRoleSelect').value);$('positionRoleSelect').value=$('floatingRoleSelect').value;reclassifyAfterEdit()};
