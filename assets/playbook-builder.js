...
const FIELD_W=560,FIELD_H=560;
const localTime=iso=>{try{return new Date(iso).toLocaleTimeString([],{hour:'numeric',minute:'2-digit',second:'2-digit'});}catch(_){return'';}};

const state={plays:[],currentId:null,selectedPlayerId:null,selectedPlayerIds:[],selectedShapeId:null,clipboard:null,tool:'select',history:[],future:[],drawStart:null,drag:null,shapeDrag:null,cloud:{team:null,session:null,channel:null,ready:false,timer:null,lastSavedAt:null,saving:false,error:null}};
...
function makeTemplatePlayers(rows,side){
  // Template definitions originated in the horizontal-field builder. On the vertical field,
  // the old across-field coordinate becomes X and the old LOS/depth coordinate becomes Y.
  return rows.map(r=>player(side,r[0],r[1],r[3],r[2]));
}
function blankPlay(){
  return{id:uid(),name:'New Matchup Plan',kind:'matchup',opponent:'',tags:[],opponentConcept:'',ourCall:'',coachingPoints:'',practice:false,offenseTemplate:'2x2',defenseTemplate:'425',players:[...makeTemplatePlayers(OFFENSE_TEMPLATES['2x2'],'offense'),...makeTemplatePlayers(DEFENSE_TEMPLATES['425'],'defense')],shapes:[],createdAt:nowIso(),updatedAt:nowIso()};
}
function current(){return state.plays.find(p=>p.id===state.currentId)||null;}
function normalizePlay(p){
  p={...p};p.id=p.id||uid();p.name=p.name||'Untitled Play';p.kind=['offense','defense','matchup'].includes(p.kind)?p.kind:'matchup';p.offenseTemplate=p.offenseTemplate||'';p.defenseTemplate=p.defenseTemplate||'';p.tags=Array.isArray(p.tags)?p.tags:String(p.tags||'').split(',').map(s=>s.trim()).filter(Boolean);p.players=Array.isArray(p.players)?p.players.map(x=>({...x,symbol:x.symbol||'auto'})):[];p.shapes=Array.isArray(p.shapes)?p.shapes.map(x=>({...x,lineStyle:x.lineStyle||((x.type==='motion')?'dashed':'solid'),width:x.width||({route:4,motion:4,block:7,stunt:5,zone:3}[x.type]||4),endpoint:x.endpoint||((x.type==='block')?'bar':(x.type==='zone'||x.type==='text')?'none':'arrow'),zonePattern:x.zonePattern||'diagonal'})):[];p.coachingPoints=p.coachingPoints||'';p.practice=!!p.practice;p.createdAt=p.createdAt||nowIso();p.updatedAt=p.updatedAt||p.createdAt;return p;
}
function loadLocal(){
...
function shapeSvg(s){const selected=s.id===state.selectedShapeId?' shape-selected':'';const x1=s.x1*(FIELD_W/100),y1=s.y1*(FIELD_H/100),x2=s.x2*(FIELD_W/100),y2=s.y2*(FIELD_H/100);const data=`data-shape-id="${esc(s.id)}"`;const width=Number(s.width)||4;const dash=s.lineStyle==='dotted'?'2 9':s.lineStyle==='dashed'?'10 7':'';
  if(s.type==='zone'){const x=Math.min(x1,x2),y=Math.min(y1,y2),w=Math.abs(x2-x1),h=Math.abs(y2-y1),pat=s.zonePattern||'diagonal';const handles=s.id===state.selectedShapeId?`<g class="zone-handles"><circle data-zone-handle="nw" data-shape-id="${esc(s.id)}" cx="${x}" cy="${y}" r="8"/><circle data-zone-handle="ne" data-shape-id="${esc(s.id)}" cx="${x+w}" cy="${y}" r="8"/><circle data-zone-handle="sw" data-shape-id="${esc(s.id)}" cx="${x}" cy="${y+h}" r="8"/><circle data-zone-handle="se" data-shape-id="${esc(s.id)}" cx="${x+w}" cy="${y+h}" r="8"/></g>`:'';return`<g><rect ${data} class="shape-hit${selected}" x="${x}" y="${y}" width="${w}" height="${h}" rx="14" fill="url(#zone-${pat})" stroke="#fff" stroke-width="${width}" ${dash?`stroke-dasharray="${dash}"`:''}/>${handles}</g>`;}
...
function renderShapes(){const p=current();if(!p)return;$('savedShapes').innerHTML=p.shapes.map(shapeSvg).join('');$('savedShapes').querySelectorAll('[data-shape-id]:not([data-zone-handle])').forEach(el=>el.addEventListener('pointerdown',shapePointerDown));$('savedShapes').querySelectorAll('[data-zone-handle]').forEach(el=>el.addEventListener('pointerdown',zoneHandlePointerDown));}
...
function shapePointerDown(ev){ev.stopPropagation();const id=ev.currentTarget.dataset.shapeId,p=current();if(state.tool==='erase'){snapshot();p.shapes=p.shapes.filter(s=>s.id!==id);state.selectedShapeId=null;markChanged();renderShapes();return;}state.selectedShapeId=id;state.selectedPlayerId=null;state.selectedPlayerIds=[];const sh=p.shapes.find(s=>s.id===id);if(state.tool==='select'&&sh?.type==='zone'){snapshot();state.shapeDrag={mode:'move',id,start:fieldPoint(ev),original:{x1:sh.x1,y1:sh.y1,x2:sh.x2,y2:sh.y2}};}renderShapes();renderPlayers();renderInspector();}

function zoneHandlePointerDown(ev){ev.stopPropagation();if(state.tool!=='select')return;const p=current(),id=ev.currentTarget.dataset.shapeId,handle=ev.currentTarget.dataset.zoneHandle,sh=p?.shapes.find(s=>s.id===id);if(!sh||sh.type!=='zone')return;state.selectedShapeId=id;state.selectedPlayerId=null;state.selectedPlayerIds=[];snapshot();state.shapeDrag={mode:'resize',id,handle,start:fieldPoint(ev),original:{x1:sh.x1,y1:sh.y1,x2:sh.x2,y2:sh.y2}};renderShapes();renderPlayers();renderInspector();}
...
function fieldPointerMove(ev){if(state.drag){const p=current(),pt=fieldPoint(ev),dx=pt.x-state.drag.start.x,dy=pt.y-state.drag.start.y;Object.entries(state.drag.originals||{}).forEach(([id,o])=>{const pl=p.players.find(x=>x.id===id);if(pl){pl.x=clamp(o.x+dx,0,100);pl.y=clamp(o.y+dy,0,100);}});renderPlayers();return;}if(state.shapeDrag){const p=current(),pt=fieldPoint(ev),d=state.shapeDrag,sh=p?.shapes.find(s=>s.id===d.id);if(!sh)return;const o=d.original;if(d.mode==='move'){const dx=pt.x-d.start.x,dy=pt.y-d.start.y,w=o.x2-o.x1,h=o.y2-o.y1;let nx1=o.x1+dx,ny1=o.y1+dy,nx2=o.x2+dx,ny2=o.y2+dy;const minx=Math.min(nx1,nx2),maxx=Math.max(nx1,nx2),miny=Math.min(ny1,ny2),maxy=Math.max(ny1,ny2);if(minx<1){nx1+=1-minx;nx2+=1-minx}if(maxx>99){nx1-=maxx-99;nx2-=maxx-99}if(miny<2){ny1+=2-miny;ny2+=2-miny}if(maxy>98){ny1-=maxy-98;ny2-=maxy-98}sh.x1=nx1;sh.x2=nx2;sh.y1=ny1;sh.y2=ny2;}else{const left=Math.min(o.x1,o.x2),right=Math.max(o.x1,o.x2),top=Math.min(o.y1,o.y2),bottom=Math.max(o.y1,o.y2);let L=left,R=right,T=top,B=bottom;if(d.handle.includes('n'))T=pt.y;if(d.handle.includes('s'))B=pt.y;if(d.handle.includes('w'))L=pt.x;if(d.handle.includes('e'))R=pt.x;if(R-L<3){if(d.handle.includes('w'))L=R-3;else R=L+3}if(B-T<3){if(d.handle.includes('n'))T=B-3;else B=T+3}sh.x1=clamp(L,1,96);sh.x2=clamp(R,4,99);sh.y1=clamp(T,2,95);sh.y2=clamp(B,5,98);}renderShapes();return;}if(state.drawStart){const pt=fieldPoint(ev);renderDraft({type:state.drawStart.type,x1:state.drawStart.x,y1:state.drawStart.y,x2:pt.x,y2:pt.y});}}
function fieldPointerUp(ev){if(state.drag){state.drag=null;markChanged();renderAll();return;}if(state.shapeDrag){state.shapeDrag=null;markChanged();renderAll();return;}if(state.drawStart){const p=current(),pt=fieldPoint(ev),d=state.drawStart;state.drawStart=null;renderDraft(null);const dist=Math.hypot(pt.x-d.x,pt.y-d.y);if(dist<1.5)return;snapshot();p.shapes.push({id:uid(),type:d.type,x1:d.x,y1:d.y,x2:pt.x,y2:pt.y,lineStyle:d.type==='motion'?'dashed':'solid',width:{route:4,motion:4,block:7,stunt:5,zone:3}[d.type]||4,endpoint:d.type==='block'?'bar':d.type==='zone'?'none':'arrow',zonePattern:'diagonal'});markChanged();renderShapes();}}

function applyTemplate(name){
  const p=current();if(!p)return;
  const isOff=!!OFFENSE_TEMPLATES[name],rows=OFFENSE_TEMPLATES[name]||DEFENSE_TEMPLATES[name];if(!rows)return;
  if(p.shapes.length&&!confirm('Applying a formation/front template will clear existing drawn lines so they do not point to old player locations. Continue?'))return;
  snapshot();
  const side=isOff?'offense':'defense';
  p.players=p.players.filter(x=>x.side!==side);
  p.players.push(...makeTemplatePlayers(rows,side));
  if(isOff)p.offenseTemplate=name;else p.defenseTemplate=name;
  p.shapes=[];state.selectedPlayerId=null;state.selectedPlayerIds=[];state.selectedShapeId=null;
  setTool('select');markChanged();renderAll();toast(`${templateLabel(name)} aligned for vertical field`);
}
function templateLabel(name){return({'2x2':'2x2','trips':'Trips Right','bunch':'Bunch Right','empty':'Empty','ace':'Ace','425':'4-2-5','43':'4-3','33':'3-3 Stack','odd':'Odd'})[name]||String(name).toUpperCase();}
function selectUnit(side){const p=current();if(!p)return;state.selectedPlayerIds=p.players.filter(x=>x.side===side).map(x=>x.id);state.selectedPlayerId=state.selectedPlayerIds[0]||null;state.selectedShapeId=null;setTool('select');renderPlayers();renderShapes();renderInspector();toast(`${side==='offense'?'Offense':'Defense'} selected — drag any selected player to move the unit`);}
function resetUnit(side){const p=current();if(!p)return;const name=side==='offense'?(p.offenseTemplate||'2x2'):(p.defenseTemplate||'425');const rows=side==='offense'?OFFENSE_TEMPLATES[name]:DEFENSE_TEMPLATES[name];if(!rows)return;if(p.shapes.length&&!confirm(`Reset ${side} alignment? Drawn routes/stunts/zones will be cleared so they stay attached to the correct locations.`))return;snapshot();p.players=p.players.filter(x=>x.side!==side);p.players.push(...makeTemplatePlayers(rows,side));p.shapes=[];state.selectedPlayerId=null;state.selectedPlayerIds=[];state.selectedShapeId=null;setTool('select');markChanged();renderAll();toast(`${side==='offense'?'Offense':'Defense'} reset to ${templateLabel(name)}`);}
function rotateLegacyLayout(){const p=current();if(!p)return;if(!confirm('Rotate this existing play from the old horizontal preset orientation into the vertical field? This rotates players and drawings 90 degrees.'))return;snapshot();p.players.forEach(pl=>{const x=pl.x;pl.x=clamp(pl.y,0,100);pl.y=clamp(x,0,100);});p.shapes.forEach(sh=>{let x=sh.x1;sh.x1=clamp(sh.y1,0,100);sh.y1=clamp(x,0,100);x=sh.x2;sh.x2=clamp(sh.y2,0,100);sh.y2=clamp(x,0,100);});state.selectedPlayerId=null;state.selectedPlayerIds=[];state.selectedShapeId=null;setTool('select');markChanged();renderAll();toast('Legacy layout rotated to vertical field');}
...
function bind(){
  $('newPlayBtn').onclick=newPlay;$('savePlayBtn').onclick=saveCurrentNow;$('duplicatePlayBtn').onclick=duplicatePlay;$('flipPlayBtn').onclick=flipPlay;$('undoBtn').onclick=undo;$('redoBtn').onclick=redo;$('selectOffenseBtn').onclick=()=>selectUnit('offense');$('selectDefenseBtn').onclick=()=>selectUnit('defense');$('resetOffenseBtn').onclick=()=>resetUnit('offense');$('resetDefenseBtn').onclick=()=>resetUnit('defense');$('rotateLegacyBtn').onclick=rotateLegacyLayout;$('printCurrentBtn').onclick=()=>printPlays(current()?[current()]:[]);$('printSetBtn').onclick=()=>printPlays(state.plays.filter(p=>p.practice));$('exportBtn').onclick=exportPlaybook;$('importFile').onchange=e=>{importPlaybook(e.target.files?.[0]);e.target.value=''};
