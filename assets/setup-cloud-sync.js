/* Analyst Assist setup cloud sync.
   - Keeps team-wide setup/preferences in team_setup_state so concepts, formations,
     branding, defensive settings, etc. can be shared by other Analyst Assist pages.
   - Full Coaching Console game/workspace state (including film + live play logs)
     is now synced separately by analyst-assist-sync.js through live_game_state.
...
(function () {
  ...
  const SETUP_KEYS = [
    'opp','week','teamName','primary','secondary','accent','weight','groupForms',
    'active','customConcepts','deletedConcepts','customFormations','deletedFormations',
    'activeFormations','formationActivationInitialized','scoutNotes','defense','star',
    '_setupUpdatedAt'
  ];
  ...
  function captureSetupState() {
    ...
    SETUP_KEYS.forEach((key) => {
      if (typeof ST !== 'undefined' && Object.prototype.hasOwnProperty.call(ST, key)) {
  ...
  function rebuildConceptList() {
    if (typeof CONCEPTS === 'undefined' || typeof DEFAULT_CONCEPTS === 'undefined') return;
    CONCEPTS.length = 0;
    DEFAULT_CONCEPTS.forEach((concept) => CONCEPTS.push([...concept]));
    (ST.customConcepts || []).forEach((concept) => {
      if (!concept || !concept.name) return;
      const alreadyExists = CONCEPTS.some(
        (item) => item[0].toLowerCase() === String(concept.name).toLowerCase()
      );
      if (!alreadyExists) CONCEPTS.push([concept.name, concept.type || 'Run']);
    });
  }
  ...
  function applySetupState(incoming) {
    if (!incoming || typeof incoming !== 'object' || typeof ST === 'undefined') return;
    applyingRemote = true;
    try {
      SETUP_KEYS.forEach((key) => {
        if (Object.prototype.hasOwnProperty.call(incoming, key)) ST[key] = clone(incoming[key]);
      });
      normalizeSetupState();
      rebuildConceptList();
  ...
  async function saveCloudNow() {
    const client = getClient();
    if (!client || !session || !team || applyingRemote) return;
    const { error } = await client.from('team_setup_state').upsert({
      team_id: team.id,
    ...
    // saveState normally already queues the full workspace sync. This extra hook
    // covers setup-only writes coming from another page/function.
    if (typeof window.queueFullStateCloudSave === 'function') window.queueFullStateCloudSave();
  ...
  async function loadOrCreateCloudState(client) {
    const { data, error } = await client
      .from('team_setup_state').select('team_id,state,device_id,updated_at')
    ...
    const remoteTime = remoteState ? (Date.parse(remoteState._setupUpdatedAt || data.updated_at || '') || 0) : 0;
  ...
  function subscribe(client) {
    ...
    channel = client.channel('aa-team-setup-' + team.id)
      .on('postgres_changes', {
        event:'*', schema:'public', table:'team_setup_state', filter:'team_id=eq.' + team.id
      }, (payload) => {
        const row = payload.new;
        if (!row || !row.state || row.device_id === deviceId) return;
  ...
  async function init() {
    const client = getClient();
    if (!client) { console.warn('Setup cloud sync could not find window.aaSupabase.'); return; }
    try {
      session = await getSession();
      if (!session) { console.warn('Setup cloud sync could not find an authenticated session.'); return; }
      team = await pickTeam(client);
      if (!team) { console.warn('Setup sync could not find a team row for this login.'); return; }
      initialized = true;
      await loadOrCreateCloudState(client);
      subscribe(client);
    } catch (error) {
      console.warn('Analyst Assist setup sync failed:', error);
      initialized = false;
    }
  }
